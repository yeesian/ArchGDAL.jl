...
- Feature layer iteration resolves the layer's column layout once, which `Tables.getcolumn` reads by name and by index.
- `Tables.getcolumn(row, i)` indexes the columns in `Tables.columnnames` order — the FID, then the geometry fields, then the ordinary fields — where it used to reach the ordinary fields before the geometry ones.
