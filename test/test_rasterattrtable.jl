...
@testset "test_rasterattrtable.jl" begin
    @testset "Testing Raster Attribute Tables" begin
        AG.createRAT() do rat
            ...
            @test (@test_logs (:warn, "There is no linear binning information.") AG.getlinearbinning(rat)) == (0, 10)
            AG.setlinearbinning!(rat, -1.5, 12.0)
            @test (@test_logs (:warn, "There is no linear binning information.") AG.getlinearbinning(rat)) == (-1.5, 12.0)
