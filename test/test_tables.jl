        ...
        @testset "Conversion to table for drivers: GeoJSON, ESRI Shapefile" begin
            ...
            function get_test_dataset(
                ...
                filename = joinpath(
                    @__DIR__,
                    "tmp",
                    filename_root * ds_file_extension[drvshortname],
                )
                ...
                for (drvshortname, file_extension) in ds_file_extension
                    ...
                    isfile(joinpath(@__DIR__, "tmp", filename_root * ".xsd")) &&
                        rm(joinpath(@__DIR__, "tmp", filename_root * ".xsd"))
                    isfile(joinpath(@__DIR__, "tmp", filename_root * ".tmp")) &&
                        rm(joinpath(@__DIR__, "tmp", filename_root * ".tmp"))
                    isdir(joinpath(@__DIR__, "tmp", filename_root)) && rm(
                        joinpath(@__DIR__, "tmp", filename_root),
                        recursive = true,
                    )
                end
