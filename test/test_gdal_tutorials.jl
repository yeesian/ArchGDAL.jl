...
AG.registerdrivers() do
    facts("Raster Tutorial") do
        AG.read("data/utmsmall.tif") do dataset
            ...
            @fact AG.getgeotransform(dataset) --> roughly([440720.0,60.0,0.0,3.75132e6,0.0,-60.0])

            band = AG.getband(dataset, 1)
            @fact AG.getblocksize(band) --> roughly([100, 81])
            @fact AG.getdatatype(band) --> UInt8
            @fact AG.colorname(AG.getcolorinterp(band)) --> "Gray"
        ...
        AG.read("data/utmsmall.tif") do ds_src
            AG.write("tmp/utmsmall.tif", ds_src)
        end
        rm("tmp/utmsmall.tif")
    end
    ...
    facts("Vector Tutorial") do
        AG.read("data/point.geojson") do dataset
            ...
            layerbyname = AG.borrow_getlayer(dataset, "OGRGeoJSON")
            @fact layerbyname --> layer
            AG.resetreading!(layer)

            featuredefn = AG.borrow_getlayerdefn(layer)
            @fact AG.nfield(featuredefn) --> 2
            fielddefn = AG.borrow_getfielddefn(featuredefn, 0)
            @fact GDAL.gettype(fielddefn) --> GDAL.OFTReal
            fielddefn = AG.borrow_getfielddefn(featuredefn, 1)
            ...
            AG.nextfeature(layer) do feature # second feature
                ...
                geometry = AG.borrow_getgeom(feature)
                @fact AG.getgeomname(geometry) --> "POINT"
                @fact AG.getgeomtype(geometry) --> GDAL.wkbPoint
                @fact AG.nfield(featuredefn) --> 2
